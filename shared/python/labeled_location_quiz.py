...
// Mobile if narrow (portrait phone) OR short (landscape phone). Tablet landscape
// (height >= ~768) stays desktop, as required.
const mq = window.matchMedia ? window.matchMedia('(max-width:760px), (max-height:500px)') : null;
...
window.addEventListener('orientationchange', () => setTimeout(() => { setHeaderTop(); applyMobileMode(); fit(); }, 150));
setHeaderTop();
applyMobileMode();
fit();
</script>
