 ...
 *{box-sizing:border-box}
 ...
 body.mobile #mobileMenuBtn{display:inline-flex}
 /* Back-to-list is only an option BEFORE the term is placed (slot kept to avoid shift). */
 body.mobile #mobileBack{display:inline-flex;visibility:hidden}
 body.mobile.m-map.m-unplaced #mobileBack{visibility:visible}
 /* ⋯ menu */
 ...
 #mPotvrdi:disabled,#mNext:disabled{opacity:.45;border-color:#bbb;color:#bbb;background:#fff;cursor:default}
...
panelBody.addEventListener('click', e => {
  ...
  mUpdateNextState();   // enable "Sledeći pojam" once all questions are answered (test, mobile)
  saveState();
  maybeAutoAnalyze();
});
...
function setTestMode(on, opts) {
  ...
  if (typeof setHeaderTop === 'function') setHeaderTop();   // test mode adds a stats row → re-measure header
  if (typeof mUpdateNextState === 'function') mUpdateNextState();
  saveState();
}
...
function mGoto(screen) {
  ...
  mHideHint();
  if (screen !== 'map') { mProvisional = null; mAwaitNext = false; mPotvrdi.disabled = true; mPotvrdi.textContent = M_CONFIRM_TXT; mTermLabel.textContent = ''; document.body.classList.remove('m-unplaced'); }
  else { fit(); }   // map just became visible — measure it and fit the whole map
  if (screen === 'result') mUpdateNextState();
  mCloseMenu();
  window.scrollTo(0, 0);
}
...
// In Test mode the student returns to the list only after every question on the
// result screen is answered; in Learn mode "Sledeći pojam" is always available.
function mAllAnswered(id) {
  const p = getQP(id);
  return p.solved.length > 0 && p.solved.every((s, i) => s || p.wrong[i]);
}
function mUpdateNextState() {
  if (!MOBILE()) return;
  if (!isTestMode()) { mNext.disabled = false; return; }
  const q = panelBody.querySelector('.quiz');
  const id = q ? parseInt(q.dataset.canon, 10) : null;
  mNext.disabled = !(id != null && mAllAnswered(id));
}
...
function mConfirm() {
  ...
  try {
    ...
    mProvisional = null;
    document.body.classList.remove('m-unplaced');   // placed → back-to-list no longer offered
...
legendList.addEventListener('click', e => {
  ...
  mGoto('map');
  document.body.classList.add('m-unplaced');   // back-to-list allowed only until placed
  mSetTermLabel(id);
});
...
// Order matters: applyMobileMode() sets the header's mobile padding, THEN we
// measure it for --htop, so the fixed content sits flush under the real header.
window.addEventListener('resize', () => { applyMobileMode(); setHeaderTop(); fit(); });
window.addEventListener('orientationchange', () => setTimeout(() => { applyMobileMode(); setHeaderTop(); fit(); }, 200));
applyMobileMode();
setHeaderTop();
fit();
</script>
